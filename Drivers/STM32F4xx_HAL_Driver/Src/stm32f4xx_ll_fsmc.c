...
HAL_StatusTypeDef FSMC_NORSRAM_Timing_Init(FSMC_NORSRAM_TypeDef *Device,
  ...
  uint32_t tmpr;
	...
	MODIFY_REG(Device->BTCR[Bank + 1U], BTR_CLEAR_MASK,
			(Timing->AddressSetupTime | ((Timing->AddressHoldTime) << FSMC_BTR1_ADDHLD_Pos) | ((Timing->DataSetupTime) << FSMC_BTR1_DATAST_Pos) | ((Timing->BusTurnAroundDuration) << FSMC_BTR1_BUSTURN_Pos) | (((Timing->CLKDivision) - 1U) << FSMC_BTR1_CLKDIV_Pos) | (((Timing->DataLatency) - 2U) << FSMC_BTR1_DATLAT_Pos) | (Timing->AccessMode)));
...
FSMC_NORSRAM_EXTENDED_TypeDef *Device, FSMC_NORSRAM_TimingTypeDef *Timing,
	...
	if (ExtendedMode == FSMC_EXTENDED_MODE_ENABLE) {
		...
		MODIFY_REG(Device->BWTR[Bank], BWTR_CLEAR_MASK,
				(Timing->AddressSetupTime | ((Timing->AddressHoldTime) << FSMC_BWTR1_ADDHLD_Pos) | ((Timing->DataSetupTime) << FSMC_BWTR1_DATAST_Pos) | Timing->AccessMode | ((Timing->BusTurnAroundDuration) << FSMC_BWTR1_BUSTURN_Pos)));
	} else {
...
HAL_StatusTypeDef FSMC_NAND_Init(FSMC_NAND_TypeDef *Device,
	...
	if (Init->NandBank == FSMC_NAND_BANK2) {
		...
		MODIFY_REG(Device->PCR2, PCR_CLEAR_MASK,
				(Init->Waitfeature | FSMC_PCR_MEMORY_TYPE_NAND | Init->MemoryDataWidth | Init->EccComputation | Init->ECCPageSize | ((Init->TCLRSetupTime) << FSMC_PCR2_TCLR_Pos) | ((Init->TARSetupTime) << FSMC_PCR2_TAR_Pos)));
	} else {
...
HAL_StatusTypeDef FSMC_NAND_CommonSpace_Timing_Init(FSMC_NAND_TypeDef *Device,
	...
	if (Bank == FSMC_NAND_BANK2) {
		...
		MODIFY_REG(Device->PMEM2, PMEM_CLEAR_MASK,
				(Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PMEM2_MEMWAIT2_Pos) | ((Timing->HoldSetupTime) << FSMC_PMEM2_MEMHOLD2_Pos) | ((Timing->HiZSetupTime) << FSMC_PMEM2_MEMHIZ2_Pos)));
	} else {
		/* NAND bank 3 registers configuration */
...
FSMC_NAND_TypeDef *Device, FSMC_NAND_PCC_TimingTypeDef *Timing, uint32_t Bank) {
	/* Check the parameters */
	assert_param(IS_FSMC_NAND_DEVICE(Device));
	assert_param(IS_FSMC_SETUP_TIME(Timing->SetupTime));
	assert_param(IS_FSMC_WAIT_TIME(Timing->WaitSetupTime));
	...
	if (Bank == FSMC_NAND_BANK2) {
		...
		MODIFY_REG(Device->PATT2, PATT_CLEAR_MASK,
				(Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PATT2_ATTWAIT2_Pos) | ((Timing->HoldSetupTime) << FSMC_PATT2_ATTHOLD2_Pos) | ((Timing->HiZSetupTime) << FSMC_PATT2_ATTHIZ2_Pos)));
	} else {
		/* NAND bank 3 registers configuration */
...
HAL_StatusTypeDef FSMC_PCCARD_Init(FSMC_PCCARD_TypeDef *Device,
	...
	MODIFY_REG(Device->PCR4,
			...
			(FSMC_PCR_MEMORY_TYPE_PCCARD | Init->Waitfeature | FSMC_NAND_PCC_MEM_BUS_WIDTH_16 | (Init->TCLRSetupTime << FSMC_PCR4_TCLR_Pos) | (Init->TARSetupTime << FSMC_PCR4_TAR_Pos)));
...
FSMC_PCCARD_TypeDef *Device, FSMC_NAND_PCC_TimingTypeDef *Timing) {
	...
	MODIFY_REG(Device->PMEM4, PMEM4_CLEAR_MASK,
			(Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PMEM4_MEMWAIT4_Pos) | ((Timing->HoldSetupTime) << FSMC_PMEM4_MEMHOLD4_Pos) | ((Timing->HiZSetupTime) << FSMC_PMEM4_MEMHIZ4_Pos)));
...
FSMC_PCCARD_TypeDef *Device, FSMC_NAND_PCC_TimingTypeDef *Timing) {
	...
	MODIFY_REG(Device->PATT4, PATT4_CLEAR_MASK,
			(Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PATT4_ATTWAIT4_Pos) | ((Timing->HoldSetupTime) << FSMC_PATT4_ATTHOLD4_Pos) | ((Timing->HiZSetupTime) << FSMC_PATT4_ATTHIZ4_Pos)));
...
HAL_StatusTypeDef FSMC_PCCARD_IOSpace_Timing_Init(FSMC_PCCARD_TypeDef *Device,
	...
	MODIFY_REG(Device->PIO4, PIO4_CLEAR_MASK,
			(Timing->SetupTime | (Timing->WaitSetupTime << FSMC_PIO4_IOWAIT4_Pos) | (Timing->HoldSetupTime << FSMC_PIO4_IOHOLD4_Pos) | (Timing->HiZSetupTime << FSMC_PIO4_IOHIZ4_Pos)));
