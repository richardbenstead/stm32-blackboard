...
void DiffBuffBase::rotationBox(int orientation, int xmin, int xmax, int ymin,
		int ymax, int &x1, int &x2, int &y1, int &y2) {
	switch (orientation) {
...
void DiffBuffBase::copyfb(uint16_t *fb_old, const uint16_t *fb_new,
		int fb_new_orientation) {
...
void DiffBuffBase::copyfb(uint16_t *fb_old, const uint16_t *fb_new, int xmin,
		int xmax, int ymin, int ymax, int src_stride, int fb_new_orientation) {
	int x1, x2, y1, y2;
...
void DiffBuffBase::_copy_rotate_0(uint16_t *fb_dest, const uint16_t *fb_src) {
	memcpy(fb_dest, fb_src,
			sizeof(uint16_t) * DiffBuffBase::LX * DiffBuffBase::LY);
}
...
void DiffBuffBase::_copy_rotate_0(uint16_t *fb_dest, const uint16_t *fb_src,
		int x1, int x2, int y1, int y2, int w, int h, int src_stride) {
...
void DiffBuff::_computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
		int fb_new_orientation, int gap, uint16_t compare_mask) {
	switch (fb_new_orientation) {
	case PORTRAIT_240x320:
		_computeDiff0<COPY_NEW_OVER_OLD, USE_MASK>(fb_old, fb_new, gap,
				compare_mask);
		return;
	case LANDSCAPE_320x240:
		_computeDiff1<COPY_NEW_OVER_OLD, USE_MASK>(fb_old, fb_new, gap,
				compare_mask);
		return;
	case PORTRAIT_240x320_FLIPPED:
		_computeDiff2<COPY_NEW_OVER_OLD, USE_MASK>(fb_old, fb_new, gap,
				compare_mask);
		return;
	case LANDSCAPE_320x240_FLIPPED:
		_computeDiff3<COPY_NEW_OVER_OLD, USE_MASK>(fb_old, fb_new, gap,
				compare_mask);
		return;
	}
	// hum...
...
template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
void DiffBuff::_computeDiff0(uint16_t *fb_old, const uint16_t *fb_new, int gap,
...
template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
void DiffBuff::_computeDiff1(uint16_t *fb_old, const uint16_t *fb_new, int gap,
		uint16_t compare_mask) {
	int cgap = 0;   // current gap size;
	int pos = 0;    // number of pixel written in diffbuf
	int n = 0;      // current offset
...
template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
void DiffBuff::_computeDiff2(uint16_t *fb_old, const uint16_t *fb_new, int gap,
		uint16_t compare_mask) {
	int cgap = 0;   // current gap size;
	int pos = 0;    // number of pixel written in diffbuf
	int n = 0;      // current offset
...
template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
void DiffBuff::_computeDiff3(uint16_t *fb_old, const uint16_t *fb_new, int gap,
		uint16_t compare_mask) {
	int cgap = 0;   // current gap size;
	int pos = 0;    // number of pixel written in diffbuf
	int n = 0;      // current offset
...
void DiffBuff::computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
		int fb_new_orientation, int gap, bool copy_new_over_old,
		uint16_t compare_mask) {

	if (gap < 1)
		gap = 1;
	if ((fb_new_orientation < 0) || (fb_new_orientation > 3))
		fb_new_orientation = 0;
...
//                initRead();
	...
	if ((compare_mask != 0) && (compare_mask != 0xffff)) {
		if (copy_new_over_old)
			_computeDiff<true, true>(fb_old, fb_new, fb_new_orientation, gap,
					compare_mask);
		else
			_computeDiff<false, true>(fb_old, fb_new, fb_new_orientation, gap,
					compare_mask);
	} else {
		if (copy_new_over_old)
			_computeDiff<true, false>(fb_old, fb_new, fb_new_orientation, gap,
					compare_mask);
		else
			_computeDiff<false, false>(fb_old, fb_new, fb_new_orientation, gap,
					compare_mask);
	}

	_write_encoded(TAG_END);
	if ((unsigned int) size() >= (unsigned int) _sizebuf) { // diff is full so copy from new to old may not have been completed...
		if (copy_new_over_old)
			copyfb(fb_old, fb_new, fb_new_orientation); // copy again.
	}
//            initRead();
}

int DiffBuff::readDiff(int &x, int &y, int &len, int scanline) {
	if (!_r_cont) { // we must load a new instruction.
		int nb_write, nb_skip;
		while (1) {
			nb_write = _read_encoded(_posr);         // number of pixel to write
			if (nb_write == TAG_END)
				return -1; // done !
			if (nb_write == TAG_WRITE_ALL) { // must write everything
	...
	x = _r_x;
	y = _r_y;
	if ((scanline < DiffBuffBase::LY)
			&& (_r_y + MIN_SCANLINE_SPACE > scanline)) { // we must wait a bit.
		len = 0;
		const int l = _r_y + MIN_SCANLINE_SPACE;
		return ((l < DiffBuffBase::LY) ? l : DiffBuffBase::LY);
	}
	if (_r_x > 0) { // not at the beginning of a line.
		if (_r_x + _r_len <= DiffBuffBase::LX) { // everything fits on the line
			len = _r_len;
			_r_cont = false;
			return 0;
		}
	...
	if (maxl > MAX_WRITE_LINE)
		maxl = MAX_WRITE_LINE; // clamp at max value.
	const int nbw = maxl * DiffBuffBase::LX; // max number of pixels that we can write
	if (_r_len <= nbw) { // ok, we can write everything now
		len = _r_len;
		_r_cont = false;
		return 0;
	}
...
void DiffBuff::computeDiff(uint16_t *fb_old, DiffBuffBase *diff_old,
		const uint16_t *sub_fb_new, int xmin, int xmax, int ymin, int ymax,
		int stride, int fb_new_orientation, int gap, bool copy_new_over_old,
		uint16_t compare_mask) {
	if (gap < 1)
		gap = 1;
	if ((fb_new_orientation < 0) || (fb_new_orientation > 3))
...
//                initRead();
	...
	int x1, x2, y1, y2;
	DiffBuffBase::rotationBox(fb_new_orientation, xmin, xmax, ymin, ymax, x1,
			x2, y1, y2);

	_computeDiff(fb_old, diff_old, sub_fb_new, x1, x2, y1, y2, stride,
			fb_new_orientation, gap, copy_new_over_old, compare_mask);

	_write_encoded(TAG_END);
	if ((unsigned int) size() >= (unsigned int) _sizebuf) { // diff is full so copy from new to old may not have been completed...
		if (copy_new_over_old)
			copyfb(fb_old, sub_fb_new, xmin, xmax, ymin, ymax, stride,
					fb_new_orientation); // copy again.
	}
}

void DiffBuff::_computeDiff(uint16_t *fb_old, DiffBuffBase *diff_old,
		const uint16_t *sub_fb_new, int xmin, int xmax, int ymin, int ymax,
		int stride, int fb_new_orientation, int gap, bool copy_new_over_old,
		uint16_t compare_mask) {
	DiffBuffDummy dd;
...

	{
		...
		while (cur < targetpos) {
			if (nb_write > 0) {
				if (cgap >= gap) {
					if (!_write_chunk(cur - prv - cgap, cgap))
						return;
					prv = cur;
				}
				cgap = 0;
	...
	for (int yc = ymin; yc <= ymax; yc++) {
		...
		if (nb_write + nb_skip == 0)
			diff_old->readRaw(nb_write, nb_skip); // reload if needed

		const int nend = xmax + (DiffBuffBase::LX * yc);
		for (int n = xmin + (DiffBuffBase::LX * yc); n <= nend;
				n++, m += mdelta) {
			if (((fb_old[n]) ^ (sub_fb_new[m])) & compare_mask) {
				...
				if (cgap >= gap) {
					if (!_write_chunk(n - prv - cgap, cgap))
						return;
					prv = n;
				}
				cgap = 0;
			...
			} else if (nb_write > 0) { // same pixel, nb_write > 0
				if (cgap >= gap) {
					if (!_write_chunk(n - prv - cgap, cgap))
						return;
					prv = n;
				...
				cgap = 0;
				nb_write--;
			} else { // same pixel, nb_write=0 and nb_skip>0
				cgap++;
				nb_skip--;
				if (nb_skip == 0)
					diff_old->readRaw(nb_write, nb_skip); // reload if needed
...
int DiffBuffDummy::readDiff(int &x, int &y, int &len, int scanline) {
	if (_current_line >= _end)
		return -1; // we are done.
	if (scanline >= _end) { // scanline after end of drawing, go as fast as possible.
		x = 0;
		y = _current_line;
	...
	if (maxl < MIN_SCANLINE_SPACE) { // we must wait a bit.
	...
	y = _current_line;
	if (maxl > MAX_WRITE_LINE)
		maxl = MAX_WRITE_LINE; // not too much lines at once
