...
public:
	 ...
	 **/
	virtual void computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
			int fb_new_orientation, int gap, bool copy_new_over_old,
			uint16_t compare_mask) = 0;
	 ...
	 *
	 **/
	virtual void computeDiff(uint16_t *fb_old, DiffBuffBase *diff_old,
			const uint16_t *sub_fb_new, int xmin, int xmax, int ymin, int ymax,
			int stride, int fb_new_orientation, int gap,
			bool copy_new_over_old = true, uint16_t compare_mask = 0) = 0;

	/**
	 * Copy the new framebuffer over the old one (and rotate it to put it in orientation 0 in fb_old).
	 **/
	static void copyfb(uint16_t *fb_old, const uint16_t *fb_new,
	 ...
	 *
	 **/
	static void copyfb(uint16_t *fb_old, const uint16_t *fb_new, int xmin,
			int xmax, int ymin, int ymax, int src_stride,
			int fb_new_orientation);
	 ...
	 **/
	static void rotationBox(int orientation, int xmin, int xmax, int ymin,
...
public:
	 ...
	 **/
	DiffBuff(uint8_t *buffer, size_t sizebuf) :
			DiffBuffBase(), _tab(buffer), _sizebuf(sizebuf - PADDING), _posw(0), _posr(
					0), _posraw(0) {
	...
	virtual void computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
			int fb_new_orientation, int gap, bool copy_new_over_old,
			uint16_t compare_mask) override;
	 ...
	 **/
	int size() const {
		return ((_posw >= _sizebuf) ? (_sizebuf + PADDING) : _posw);
	}
...
private:
	...
	bool _write_chunk(uint32_t nbwrite, uint32_t nbskip) {
		if (_posw >= _sizebuf) { // running out of memory buffer
	...
	void _computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
			int fb_new_orientation, int gap, uint16_t compare_mask);

	/** called when the src framebuffer is in orientation 0 */
	template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
	void _computeDiff0(uint16_t *fb_old, const uint16_t *fb_new, int gap,
			uint16_t compare_mask);

	/** called when the src framebuffer is in orientation 1 */
	template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
	void _computeDiff1(uint16_t *fb_old, const uint16_t *fb_new, int gap,
			uint16_t compare_mask);

	/** called when the src framebuffer is in orientation 2 */
	template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
	void _computeDiff2(uint16_t *fb_old, const uint16_t *fb_new, int gap,
			uint16_t compare_mask);

	/** called when the src framebuffer is in orientation 3 */
	template<bool COPY_NEW_OVER_OLD, bool USE_MASK>
	void _computeDiff3(uint16_t *fb_old, const uint16_t *fb_new, int gap,
			uint16_t compare_mask);

	/** main method when computing partial diff */
	void _computeDiff(uint16_t *fb_old, DiffBuffBase *diff_old,
			const uint16_t *sub_fb_new, int xmin, int xmax, int ymin, int ymax,
			int stride, int fb_new_orientation, int gap, bool copy_new_over_old,
			uint16_t compare_mask);
...
public:
	...
	/** ctor */
	DiffBuffDummy() :
			DiffBuffBase(), _current_line(0), _begin(0), _end(DiffBuffBase::LY), _rawnb(
					0) {
		initRead();
		initRaw();
	}
	...
	virtual void computeDiff(uint16_t *fb_old, const uint16_t *fb_new,
			...
			uint16_t compare_mask) override
			{
		if (copy_new_over_old) { // still copy if requested.
